...
public class Camera {
    ...
    private int cameraInitVersion(int cameraId, int halVersion) {
        ...
        IBaikalServiceController baikalService = IBaikalServiceController.Stub.asInterface(
                    ServiceManager.getService(Context.BAIKAL_SERVICE_CONTROLLER));

        String packageName = ActivityThread.currentOpPackageName();

        try {
            if( baikalService != null  ) { 
                if( baikalService.getAppOption(packageName,BaikalServiceManager.OP_CAMERA_HAL1) == 1 ) {
